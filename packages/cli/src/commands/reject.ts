...
export async function rejectCommand(executionId: string, options: { comment?: string }) {
  const client = createApiClient();
  
  console.log(chalk.red(`\n❌ Rejet de: ${executionId}\n`));

  const body: any = { decision: 'rejected' };
  if (options.comment) {
